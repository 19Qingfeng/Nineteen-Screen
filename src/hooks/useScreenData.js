...
export default function() {
    ...
    const growthLastDay = ref(1);
    const growthLastMonth = ref(12.1);
    const ageData = reactive(ageMockData)
    const avgAge = ref(20)
    ...
    const saleListData = ref(salesListMockData)
    ...
    onMounted(() => {
        setInterval(() => {
            todayUser.value += Math.random() * 50
            growthLastDay.value += Math.random() + 1
            growthLastMonth.value = Math.random() * 100
            avgAge.value += Math.random() * 10
            genderData.value.forEach(i => {
                i.value += Math.random() * 100
            })
            riderData.value.orderData.data1 = riderMockData.orderData.data1.map(i => {
                i = Number(i)
                return (i = Math.random() * 200).toFixed(2)
            })
            riderData.value.orderData.data2 = riderMockData.orderData.data2.map(i => {
                i = Number(i)
                return (i = Math.random() * 200).toFixed(2)
            })

            riderData.value.rateData.data1 = riderMockData.rateData.data1.map(i => {
                i = Number(i)
                return (i = Math.random() * 200).toFixed(2)
            })
            riderData.value.rateData.data2 = riderMockData.rateData.data2.map(i => {
                i = Number(i)
                return (i = Math.random() * 200).toFixed(2)
            })
